...
Статус: целевая архитектура, принята как направление; не описание текущего
развёртывания. Реализованные компоненты и ограничения — в документе 27.

## Контуры

Диаграмма описывает целевую композицию. В текущем прототипе отсутствуют LLM
Worker, Agent Factory, Observation Collector, Observer и Learning Branch Runtime.
...
Дополнение от 2026-09-07: к Runtime и Gateways подключён проектируемый
Observation Collector. Он записывает события в независимый от Worker журнал;
Observer читает разрешённые события и версии артефактов. Этот путь наблюдения
не предоставляет управления агентами или доступа к секретам. Checkpoint
сохраняется до каждого решения/вызова; учебная ветка создаётся Runtime по
отдельному разрешению, а не самим наблюдателем.
Подробности и статус реализации: [наблюдатель](28-observer-and-learning.md).
Context Compiler собирает слоистую память по scope и сроку хранения; старые
сводки не заменяют Process State или первичные данные для критичного действия.
Политика памяти — [документ 30](30-memory-model.md).
