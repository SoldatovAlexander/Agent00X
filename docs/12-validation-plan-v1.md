...
Статус: каталог проверок. Текущий порядок R0—R5 — в документе 24; фактически
пройденные проверки — только в документе 27.
...
### VP-20. Наблюдатель и история контрольных точек

Проверить V-17/ADR-029 на агенте и одном субагенте: решение, вызов, отказ,
ошибка, задержанный результат, crash, дубликат события и конкурентная запись.
Отказ журнала до checkpoint должен запрещать действие. Проверить причинность,
срез «известно тогда» против «узнали позже», redaction и запрет изменения истории.
Отчёт ссылается на события; отсутствующее основание не заменяется догадкой.
Выход: автоматические тесты плюс отчёты на заранее заданные моменты времени.

### VP-21. Корректировка и учебная ветка

Проверить V-18/ADR-030: из ошибочного шага создать новую ветку и candidate build.
Исходная история неизменна, старые grants не действуют, внешние записи в симуляции
отсутствуют. Недоступные артефакты и неизвестный внешний исход не замалчиваются.
Отдельно оценить исходный случай, новые похожие случаи и регрессионный набор.
Критерии улучшения фиксируются до запуска; отсутствие переноса — отрицательный
результат гипотезы, даже если повтор обучающего примера успешен.

### VP-22. Исполнение функции и вклад организации данных

Проверить V-19/ADR-028 на repository-change. До запусков зафиксировать функцию,
входы, исключения, критерии результата и допустимые уточнения/отказы. Использовать
одинаковую модель и версию, инструменты, границы безопасности и сопоставимый
бюджет. Сравнить полное представление данных/процедур/состояния с упрощённым
представлением тех же исходных фактов; конкретные убираемые элементы описать
заранее. Не убирать security controls ради сравнительного опыта.

Набор содержит штатные задачи, неполные/противоречивые данные, сбои инструментов
и отказ в правах; оценка не должна опираться только на тесты, написанные Worker.
Записывать результаты всех попыток и вмешательства человека. Повторные запуски
учитывают недетерминированность модели. Выход: отчёт об исполнении функции,
вкладе представления, ограничениях и отрицательных случаях. Снижение стоимости
не является обязательным доказательством возможности исполнения.

### VP-23. Слоистая память, ретенция и суммаризация

Проверить V-20/ADR-031 на процессах разного возраста и classification. До шести
месяцев агент использует разрешённые первичные данные; от шести до двенадцати
месяцев создаётся месячная сводка; после года — годовая тематическая. Проверить
source digest, critical references, redaction, неполноту, архив/удаление,
legal-hold exception и отсутствие секретов в сводке.

Субагент не получает память родителя вне Task Contract. Сводка не заменяет
актуальный источник, policy или approval для write. При недоступном исходнике
Observer и учебная ветка сообщают границу воспроизводимости, а не придумывают
детали. Выход: схемы/тесты сводок и отчёт об ограничении контекста.

## Порядок исполнения

С 2026-09-07 порядок R0—R5 документа 24 имеет приоритет. VP-20 начинается вместе
с восстановлением целостности approval/manifest; VP-21—VP-23 завершают
исследовательский срез. Перечень ниже сохраняется как группировка старых проверок.
...
## Go/no-go для расширения исследовательского среза

### Допустимо расширение

- релевантные текущему срезу проверки пройдены без критических обходов;
