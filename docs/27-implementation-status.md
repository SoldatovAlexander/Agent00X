...
| Наблюдатель и обучение | Реализовано и unit-tested на уровне контрактов | ADR-029/030 и документ 28; схемы и негативные тесты ObservationEvent/Checkpoint/LearningCorrection/MemorySummary есть (см. раздел 9); collector, отчёты и учебные ветки отсутствуют |
...
Результат сверки 2026-09-13: **420 тестов, все прошли, 1 пропущен**. Пропущен
...
1. `build_publish_manifest` сопоставляет workspace с вновь переданным mapping,
   ...
   негативный сценарий принял подменённый после проверки файл. Устранено
   локальным fail-closed regression (см. раздел 9); сквозное доказательство
   в составе цепочки отсутствует.
2. `DeterministicPolicy.decide` проверяет approval относительно `staged_change`,
   ...
   получил `allow` для иного digest. Устранено локальным fail-closed regression
   (см. раздел 9); сквозное доказательство в составе цепочки отсутствует.
...
Пункты 1–2 устранены на уровне локальных unit-regression (см. раздел 9),
однако gate approval integrity, сквозная публикация и её recovery как единое
целое не доказаны (см. п.3 и открытые границы раздела 9). До проверок пунктов
4–5 нельзя обещать непробиваемую изоляцию или полный исторический отчёт.
...
| R1: целостность и фиксация действий | Частично проверено | Нарушения 1–2 закрыты локальными regression, checkpoint-gate реализован (см. раздел 9); следующий проверяемый результат — сквозное доказательство цепочки approval → policy → manifest → Broker → journal → recovery |
