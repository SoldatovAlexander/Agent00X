...
| Привязка/expiry/single-use broker grant | `broker.py`, `github_actuator.py` | Digest-привязка, обязательный expiry на каждом открытии канала, approval обязателен; grant остаётся reusable после failure фабрики, single-use фиксируется только после успешного открытия conforming channel | Настоящий mint/revoke токенов; отзыв и propagation expiry | Тест с controllable clock seam и fake-minter: отзыв/истечение до открытия канала, локально |
