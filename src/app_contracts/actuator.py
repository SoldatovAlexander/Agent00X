...
def publish_authorized_request(
    ...
    *,
    ...
    branch and digest. When the approval-bound ``intent`` is provided, the
    request branch and idempotency key must additionally equal the approved
    values, so a post-allow swap of branch or key is rejected here instead
    of reaching the mock boundary.
    """
