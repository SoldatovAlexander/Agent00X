...
class CanaryCredentialTests(unittest.TestCase):
    ...
    def test_empty_surface_name_rejected_before_diagnostics(self):
        surfaces = {"": {"note": self.canary}, "stdout": "clean"}
        with self.assertRaises(ValueError) as raised:
            find_canary_surfaces(self.canary, surfaces)
        self.assertEqual(str(raised.exception), "surface name must be a non-empty string")
        self.assertNotIn(self.canary, str(raised.exception))
        with self.assertRaises(ValueError):
            assert_canary_absent(self.canary, surfaces)
        self.assertEqual(
            find_canary_surfaces(self.canary, {"stdout": "clean"}), []
        )
    ...
    def test_non_string_surface_key_is_rejected_without_repr(self):
        for hostile in (123, ("agent", "context"), frozenset({"agent-context"}), ""):
            with self.subTest(key=repr(hostile)):
                ...
                message = str(raised.exception)
                self.assertEqual(message, "surface name must be a non-empty string")
                self.assertNotIn(self.canary, message)
        with self.assertRaises(ValueError):
